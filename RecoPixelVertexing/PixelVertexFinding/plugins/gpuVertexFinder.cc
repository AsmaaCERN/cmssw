...
// #define THREE_KERNELS
  ...
  ZVertexHeterogeneous Producer::make(TkSoA const* tksoa, float ptMin, float ptMax) const {
    Producer::testVertexFit3D(tksoa, ptMin, ptMax); 
#ifdef PIXVERTEX_DEBUG_PRODUCE
...
#ifdef __CUDACC__
  ZVertexHeterogeneous Producer::testVertexFit3DAsync(cudaStream_t stream, TkSoA const* tksoa, float ptMin, float ptMax) const {
#ifdef PIXVERTEX_DEBUG_PRODUCE
    std::cout << "producing Vertices on GPU" << std::endl;
#endif  // PIXVERTEX_DEBUG_PRODUCE
    ZVertexHeterogeneous vertices(cms::cuda::make_device_unique<ZVertexSoA>(stream));
#else
  ZVertexHeterogeneous Producer::testVertexFit3D(TkSoA const* tksoa, float ptMin, float ptMax) const {
    std::cout << "[gpuVertexFit3D] New function running! " << std::endl; 
#ifdef PIXVERTEX_DEBUG_PRODUCE
    std::cout << "producing Vertices on  CPU" << std::endl;
#endif  // PIXVERTEX_DEBUG_PRODUCE
    ZVertexHeterogeneous vertices(std::make_unique<ZVertexSoA>());
